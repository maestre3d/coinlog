...
type CardQuery struct {
	...
	withUser             *UserQuery
	withFinancialAccount *FinancialAccountQuery
	withFKs              bool
...
func (cq *CardQuery) QueryFinancialAccount() *FinancialAccountQuery {
	query := (&FinancialAccountClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CardQuery) Clone() *CardQuery {
	...
	return &CardQuery{
		...
		withFinancialAccount: cq.withFinancialAccount.Clone(),
...
// WithFinancialAccount tells the query-builder to eager-load the nodes that are connected to
// the "financial_account" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CardQuery) WithFinancialAccount(opts ...func(*FinancialAccountQuery)) *CardQuery {
	query := (&FinancialAccountClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withFinancialAccount = query
	return cq
}
...
func (cq *CardQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Card, error) {
	var (
		...
		loadedTypes = [2]bool{
			cq.withUser != nil,
			cq.withFinancialAccount != nil,
		}
	)
	if cq.withUser != nil || cq.withFinancialAccount != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withFinancialAccount; query != nil {
		if err := cq.loadFinancialAccount(ctx, query, nodes, nil,
			func(n *Card, e *FinancialAccount) { n.Edges.FinancialAccount = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CardQuery) loadFinancialAccount(ctx context.Context, query *FinancialAccountQuery, nodes []*Card, init func(*Card), assign func(*Card, *FinancialAccount)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Card)
	for i := range nodes {
		fk := nodes[i].FinancialAccountID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	}
	query.Where(financialaccount.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "financial_account_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
